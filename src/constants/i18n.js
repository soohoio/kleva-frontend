export const en = {
  ...
  "glossary.13.description": `ib Token stands for interest bearing token that accumulates interest by just holding it. <br/>When user deposit token to KLEVA protocol, they are given ib token as a proof of deposit. The amount user receieves depends on the conversion ratio. For example, with the conversion ratio of 1 ibWEMIX = 1 WEMIX, user will receive 100 ibWEMIX for depositing 100 WEMIX. The token deposited is lended to farmers. <br/>Farms in return pay borrowing interest, which is used to increase the ib token conversion ratio. So carrying on with the example, the conversion ratio will rise to 1 ibWEMIX - 1.001WEMIX, increasing the value of ib token. <br/>As a result when user decides to make a withdrawal, depositors will be given more token then they first deposited. In addition those who staked ib tokens will be able to earn KLEVA token reward. `,
  ...
  "glossary.17.description": `Position value explains the status of the farm position. Current USD value and the amount of tokens in the position is displayed. <br/>* Position value = equity value + debt value`,
  ...
  "glossary.20.description": `Debt ratio is a ratio of debt value to position value. If debt ratio gets to high, borrowed asset is thought to be at risk. Thus to protect the asset the position may get automatically closed and be liquidated. <br/>* Debt ratio = debt value / total asset`,
