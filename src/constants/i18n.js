export const en = {
  ...
  'changeNetwork.title': 'Switch Network to Klaytn Mainnet',
  'changeNetwork.description': 'Launch Metamask to switch network.',
  ...
  "glossary.1.description": `Every token requires one-time token approval prior to the first use on the protocol. The purpose is to give protocol aceess to user's wallet in order to validate the wallet balance and transfer tokens that user's want to make transaction of. In the process of token approval, small amount of KLAY is needed as a gas fee.`,
  ...
  "glossary.13.description": `ib Token stands for interest bearing token that accumulates interest by just holding it. When user deposit token to KLEVA protocol, they are given ib token as a proof of deposit. The amount user receieves depends on the conversion ratio. For example, with the conversion ratio of 1 ibWEMIX = 1 WEMIX, user will receive 100 ibWEMIX for depositing 100 WEMIX. The token deposited is lended to farmers. Farms in return pay borrowing interest, which is used to increase the ib token conversion ratio. So carrying on with the example, the conversion ratio will rise to 1 ibWEMIX - 1.001WEMIX, increasing the value of ib token. As a result when user decides to make a withdrawal, depositors will be given more token then they first deposited. In addition those who staked ib tokens will be able to earn KLEVA token reward. `,
  "glossary.farming": `Leveraged Yield Farming`,
  ...
  "glossary.17.description": `Position value explains the status of the farm position. Current USD value and the amount of tokens in the position is displayed. * Position value = equity value + debt value`,
  ...
  "glossary.20.description": `Debt ratio is a ratio of debt value to position value. If debt ratio gets to high, borrowed asset is thought to be at risk. Thus to protect the asset the position may get automatically closed and be liquidated. * Debt ratio = debt value / total asset`,
  ...
  "glossary.22.description": `Liquidation is having farming position automatically closed withdrawing all asset when the meets certain condition. The condition is when user's leveraged yeild farming position's debt ratio meets the liquidation threshold. In such condition user's borrowed asset is believed to be at risk and the user may not be able to repay the debt. `,
  ...
  'farming.yieldFarmingAPR': 'Yield Farming APR',
  ...
  'borrow': 'Borrow %{title}',
  'myasset.farming.equityValue': 'Equity',
  'farm': 'FARM',
  'cancel': 'Cancel',
...
export const ko = {
  ...
  'faq.3.description': "연간 수익률은 해당 토큰 풀의 대여율에 따라 정해진 규칙에 의해 결정됩니다.",
  'faq.4.description': '클레바 프로토콜에서는 언제나 자유로운 출금이 가능합니다.',
  'faq.3.q': '연간 수익률은 어떻게 계산되나요? ',
  ...
  "glossary.3.description": "1) 연간수익률(단리) : 연간수익률(단리)는 APR이라고 표현되며, 예치한 자산에 대한 이자 수익을 계속 모으면 얻을 수 있는 이자율입니다. (하루 이자율 * 365) 2) 연간수익률(복리) : 연간수익률(복리)는 APY이라고 표현되며, 예치한 자산에 대한 이자 수익을 매일 재투자했을 때 얻을 수 있는 이자율입니다. (하루 이자율 +1 / 100)^365",
  "glossary.4.title": `프라이스 임팩트`,
  "glossary.4.description": `프라이스 임팩트는 스왑의 규모가 커짐에 따라 토큰 교환비율에 주는 영향이 커져 발생하게 됩니다. 이에 따라 유저는 거래 체결 시점에 예상보다 적은 수량의 토큰을 수령하게 되어 손실을 보게 됩니다. KLEVA Protocol 레버리지 투자에서는 두 토큰의 가치가 50:50으로 동일한 상태로 예치가 진행됩니다. 만일 유저가 예치한 두 토큰의 가치가 다를 경우, 스왑을 통해 맞추는 과정이 필요합니다. 이때 KLEVA Protocol은 가치가 더 높은 토큰을 활용해 더 낮은 토큰으로 스왑합니다. 이 과정에서 프라이스 임팩트가 발생할 수 있습니다. `,
  "glossary.5.title": `슬리피지`,
  "glossary.5.description": `슬리피지는 주문 과정에서 유저에게 안내된 토큰 가격과 실제 체결된 토큰 가격의 차이를 의미합니다. 이런 차이는 거래 체결 직전 외부 요인으로 토큰 교환비율이 변동된 경우에 발생합니다. 이에 따라 유저가 실제로 획득한 토큰 수량이 변동될 수 있습니다.  따라서 슬리피지로 인한 손실을 방지하기 위해 유저는 슬리피지 %를 설정할 수 있습니다. 예컨데 슬리피지 설정이0.5%인 경우, 획득하는 토큰 수량이 예상치로부터 0.5% 이상 편차가 생기면 거래가 성사되지 않습니다. `,
  ...
  "glossary.13.description": `ibToken (interest-bearing Token)은 이자가 쌓인다는 특징을 갖고 있는 토큰입니다. KLEVA Protocol에 토큰을 예치하면, 예치 참여자는 예치의 증표로 ib토큰을 수령할 수 있습니다.이때 유저가 받게될 ib토큰 수량은 예치 당시의 환산 비율에 따라 결정됩니다. 예를 들어 환산 비율이 1 ibWEMIX = 1 WEMIX 라면, 100WEMIX를 예치해 100ibWEMIX를 수령할 수 있습니다. 예치된 토큰은 '레버리지 투자 참여자'들에게 대여하는데 사용됩니다. 이에 따라 '레버리지 투자 참여자'들이 지불한 이자는 ib토큰의 환산 비율을 상승시키는데 사용됩니다. 예를 들어, 1ibWEMIX= 1WEMIX에서 1ibWEMIX = 1.001WEMIX 로 상승해 ibWEMIX의 가치가 상승하게 됩니다. 결과적으로 토큰 출금시, 예치 참여자들은 처음보다 많은 수량의 토큰을 지급받게 됩니다. 추가적으로 ib토큰을 스테이킹한 예치하기 참여자들은 KLEVA 토큰 보상을 받을 수 있습니다.   `,
  ...
  "glossary.17.description": `레버리지 자산에서의 총 운용 자산은 유저의 레버리지 투자 현황입니다. 자산의 달러 가치와 운용되고 있는 토큰 수량이 표기됩니다.  *총 운용 자산 = 담보자산 + 상환 필요 자산`,
  ...
  "glossary.20.description": `레버리지 비율은 총 운용 자산에서 상환 필요 자산이 차지하는 비중입니다. 레버리지 비율이 너무 높아져 유저가 대여받은 자산에 대한 책임을 다하기 어려워졌다고 판단될 경우, 유저는 '레버리지 투자' 강제 종료인 청산을 당할 수 있습니다. * 레버리지 비율 = 상환 필요 자산 / 총 운용 자산`,
  "glossary.21.title": `추가 투자`,
  "glossary.21.description": `'추가 투자'는 '레버리지 투자' 도중 '총 운용 자산'의 규모를 증가시키는 것으로 '담보 추가하기'나 '추가 대여받기'를 통해 가능합니다.   지속 가능한 투자를 위해서는 '레버리지 비율' 참고가 필요합니다. '레버리지 비율'이 높으면 담보 추가를 통해 '레버리지 비율'을 낮춰 청산의 위험을 낮출 수 있습니다. 반대로 '레버리지 비율'이 낮으면 '추가 대여받기'를 통해 자산 운용 규모를 키워 수익률을 높일 수 있습니다. `,
  ...
  "glossary.23.description": `전체 출금하기'는 특정 '레버리지 투자' 자산에 대해 유저가 투자한 자산 전체를 출금하는 기능입니다. '전체 출금하기'를 진행하면 대여받은 자산과 대여 이자인 '상환 필요 자산'은 다 상환되고, 유저는 남은 토큰을 회수하게 됩니다. 참고로 토큰 상환을 위해 토큰 스왑이 일어날 수 있으며, 이 경우 프라이스 임팩트로 인한 손실이 발생할 수 있습니다. `,
  "glossary.24.title": `부분 출금`,
  "glossary.24.description": `'부분 출금'은 특정 '레버리지 투자' 자산에 대해 투자한 자산 중 일부만 출금하는 기능입니다. 유저는 출금할 자산의 수량과 이 중 얼마를 대여받은 자산 상환에 사용할 지 정할 수 있습니다.   '부분 출금'으로 '레버리지 투자' 종료시, 발생하는 스왑의 규모를 줄일 수 있어 프라이스 임팩트로 인한 손실도 줄일 수 있습니다. `,
  ...
  'myasset.farming.withdrawnValue.description': `레버리지 비율이 청산 한도를 초과하여 담보 자산 및 대여 자산이 <text style="color: #FF5757;">강제 청산</text>되었습니다.`,
  ...
  'lendingAPR': '예치',
  'stakingAPR': '스테이킹',
  'protocolAPRFull': '프로토콜 수익률',
  'lendingAPRFull': '예치 수익률',
  'stakingAPRFull': '스테이킹 수익률',
