...
class WithdrawModal extends Component {
  ...
  renderWKLAYStep = () => {
    ...
    // || isSameAddress(stakingToken.address, tokenList.oUSDT.address)

    const isUnwrapDisabled = !this.bloc.klayAmountToUnwrap$.value
      || new BigNumber(this.bloc.klayAmountToUnwrap$.value).lte(0)
      || new BigNumber(this.bloc.klayAmountToUnwrap$.value).gt(availableWKLAYBalance)
    ...
    return (
      <>
        ...
        <InputWithPercentage
          className={cx("WithdrawModal__withdrawInput", {
            "WithdrawModal__withdrawInput--upper": true,
          })}
          decimalLimit={stakingToken.decimals}
        ...
        {isKLAY
          ...
          : (
            <>
              ...
              <InputWithPercentage
                decimalLimit={stakingToken.decimals}
                className="WithdrawModal__withdrawInput WithdrawModal__withdrawInput--common"
                value$={this.bloc.withdrawAmount$}
                valueLimit={availableBalance}
                label={stakingToken.title}
                targetToken={stakingToken}
