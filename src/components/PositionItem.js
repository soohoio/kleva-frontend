...
import { getBufferedLeverage, calcKlevaRewardsAPR, toAPY, getEachTokenBasedOnLPShare } from '../utils/calc'
...
class PositionItem extends Component {
  ...
  render() {
    ...
    const {
      ...
      poolInfo: poolInfo[lpToken && lpToken.address.toLowerCase()],
      lpShare: lpShare,
      totalShare,
      totalStakedLpBalance,
    ...
    const realPositionValue = new BigNumber(_ba)
      .multipliedBy(2)
      .toString()

    const realPositionValueParsed = new BigNumber(realPositionValue)
      .div(10 ** this.props.baseToken.decimals)
      .toNumber()

    const realEquityValueParsed = new BigNumber(realPositionValue)
      .minus(debtValue)
      .div(10 ** baseToken.decimals)
      .toNumber()
      // .toLocaleString('en-us', { maximumFractionDigits: 6 })
    ...
    return (
      <div className="PositionItem">
        ...
        <div className="PositionItem__positionValue">
          <strong>
            {nFormatter(realPositionValueParsed, 4)}
          </strong>
          {baseToken.title}
        </div>
        <div className="PositionItem__debtValue">
          <strong>{nFormatter(
            new BigNumber(debtValue)
              .div(10 ** baseToken.decimals)
              .toNumber(),
            4
          )
        ...
        <div className="PositionItem__equityValue">
          <strong>{nFormatter(realEquityValueParsed, 4)}</strong> {baseToken.title}
        </div>
